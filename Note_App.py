...
from elements.silver_scraper import Silver_Scraper
from elements.brain import NLP_opeanai
...
class LastWordFrame(wx.Frame):
    ...
    def create_archive(self, panel_stats):
        ...
        try:
            ...
            self.list = wx.ListBox(panel_stats, choices=self.files, style=wx.LB_SINGLE|wx.VSCROLL|wx.HSCROLL)
    ...
    def on_transcript(self, event):
        # merge the home_dir and the filename
        video_path = os.path.join(self.home_dir, self.filename)
        # create the transcript
        transcript = self.nlp.create_transcript(video_path)
        self.text_ctrl.SetValue(transcript)
    ...
    def InitUI(self): # create the UI - main loop
        ...
        self.show_browser = wx.Button(panel_main, label = 'o', size = (30,30))
        ...
        button_full_markdown = wx.Button(panel_main, label = 'O', size = (30,30))
        ...
        self.sizer_for_buttons_md = wx.BoxSizer(wx.VERTICAL)
        self.sizer_for_buttons_md.Add(self.show_browser, 0, wx.EXPAND)
        self.sizer_for_buttons_md.Add(button_full_markdown, 0, wx.EXPAND)
        self.text_editor_sizer.Add(self.sizer_for_buttons_md, 0, wx.EXPAND)
        ...
        def button_click_ask_AI(event):
            ...
            answer = self.nlp.generate_text(question)
            self.text_summary.SetValue(answer)
        ...
        self.media_player = wx.media.MediaCtrl(panel_main, style=wx.SIMPLE_BORDER, size = (500,300))

        # create a container for the buttons
        self.button_container = wx.BoxSizer(wx.HORIZONTAL)
        self.play_button = wx.Button(panel_main, label='Play')
        self.play_button.Disable()
        self.play_button.Bind(wx.EVT_BUTTON, self.on_play)
        self.pause_button = wx.Button(panel_main, label='Pause')
        self.pause_button.Disable()
        self.pause_button.Bind(wx.EVT_BUTTON, self.on_pause)
        self.stop_button = wx.Button(panel_main, label='Stop')
        self.stop_button.Disable()
        self.stop_button.Bind(wx.EVT_BUTTON, self.on_stop)
        self.transcript_button =  wx.Button(panel_main, label='Transcript')
        self.transcript_button.Bind(wx.EVT_BUTTON, self.on_transcript)


        self.media_container = wx.BoxSizer(wx.VERTICAL)
        self.button_container.Add(self.play_button, 0, wx.ALL)
        self.button_container.Add(self.pause_button, 0, wx.ALL)
        self.button_container.Add(self.stop_button, 0, wx.ALL)
        self.button_container.Add(self.transcript_button, 0, wx.ALL)
        self.media_container.Add(self.media_player, 1, wx.EXPAND)
        self.media_container.Add(self.button_container, 0, wx.EXPAND)


        self.left_sizer.Add(self.media_container, 1, wx.EXPAND)
        self.left_sizer.Add(text_ctrl_1, 1, wx.EXPAND)
        self.left_sizer.Hide(self.media_container)
        
        button_delete = wx.Button(panel_main, label='Delete')
        button_rename = wx.Button(panel_main, label='Rename')
        #add monoline text area
        self.text_rename = wx.TextCtrl(panel_main, style=wx.TE_PROCESS_ENTER, size = (500,300))
        ...
        def OnSelect(event):
            ...
            if str(selected_file).endswith('.txt'):
                with open(self.home_dir + '/' + selected_file, 'r') as f:
                    # read the file
                    text = f.read()
                    text_ctrl_1.SetValue(text)
                    self.left_sizer.Show(text_ctrl_1)
                    self.left_sizer.Hide(self.media_container)
            else:
                print('this must be a video file')
                self.left_sizer.Hide(text_ctrl_1)
                self.media_player.Load(self.home_dir + '/' + selected_file)
                self.left_sizer.Show(self.media_container)
                # enable the video control
                self.play_button.Enable()
                self.pause_button.Enable()
                self.stop_button.Enable()
        ...
        def OnDoubleClick(event):
            ...
            if str(selected_file).endswith('.txt'):
                with open(self.home_dir + '/' + selected_file, 'r') as f:
                    ...
                    # show the text area
                    self.left_sizer.Show(self.text_ctrl)
                    self.left_sizer.Hide(self.media_container)

            else:
                print('this must be a video file')
                # Load the selected video into the media player
                self.left_sizer.Hide(self.text_ctrl)
                self.left_sizer.Show(self.media_container)
